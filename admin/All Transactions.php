...
<body>
	...
	<form autocomplete="off" name="form1" method="POST">
		<table width="100%" border="0">
			<tr>
				<td width="15%">&nbsp;</td>
				<td width="18%" bgcolor="#CCCCCC">Borrower Id:</td>
				<td width="17%"><label for="altrid"></label> <input
					name="altrid" type="text" autofocus id="altrid" placeholder="Borrower ID" autocomplete="on"></td>
				<td width="18%" bgcolor="#CCCCCC">Type of Transactions:</td>
				<td width="17%"><label for="select"></label><select name="altrtype"
					id="altrtype" >
				  <option value="All">All</option>
				  <option value="LI">Issue</option><option value="LR">Return</option><option value="LF">Fine</option>
			  </select></td>
				<td width="15%">&nbsp;</td>
			</tr>
			<tr>
...
function fines($id,$fdt,$tdt){
	global $conn;
	$sql="select *from u490995680_lms.fines where id like '%$id%' AND date between '$fdt' AND '$tdt'";
	$result=mysqli_query($conn, $sql);
?>
	<table class="dtable" style="overflow-x: auto">
		<tr class="dtr">
			<th class="dth">Transaction Id</th>
			<th class="dth">Borrower ID</th>
			<th class="dth">Issue Transaction ID</th>
			<th class="dth">Acc No</th>
			<th class="dth">Fine</th>
			<th class="dth">New</th>
			<th class="dth">Date</th>
			<th class="dth">Authority</th>
		</tr>
<?php 	
	while($row=mysqli_fetch_assoc($result)){
?>
		<tr class="dtr">
			...
			<td class="dtd"><?php echo $row["new"]?></td>
			<td class="dtd"><?php echo $row["date"]?></td>
			<td class="dtd"><?php echo $row["asst"]?></td>
		</tr>	
<?php 		
	}
	echo "</table>";
