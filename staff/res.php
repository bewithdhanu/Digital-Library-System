...
<body>
	...
	<form autocomplete="off" name="form1" method="post" action="">
		...
		<?php 
		$count=0;
		$sql = "select count(acc) as count from u490995680_lms.books where TITL LIKE  '%$res%' AND status!='-1' OR Aut LIKE '%$res%' AND status!='-1' OR isbn LIKE '%$res%' AND status!='-1' OR acc LIKE '%$res%' AND status!='-1'";
				...
				<td>
					<table class="dtable" style="overflow-x: auto">
						<tr class="dtr">
							...
							$sql = "select *from u490995680_lms.books where TITL LIKE  '%$res%' AND status!='-1' OR Aut LIKE '%$res%' AND status!='-1' OR isbn LIKE '%$res%' AND status!='-1' OR acc LIKE '%$res%' AND status!='-1' GROUP BY ISBN";
							$result = mysqli_query ( $conn, $sql );
							if (mysqli_num_fields ( $result ) > 0) {
								while ( $row = mysqli_fetch_assoc ( $result ) ) {
									?>
